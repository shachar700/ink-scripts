...
data = data['data']['node']['weaponTopsCl']
    ...
    for edge in data['edges']:
        ...
        if node.get('weapon') and node['weapon']['weaponCategory']['name']:
            ...
            if weapon_category_name != last_category:
                file_out.write("|-\n")
                file_out.write("! colspan=\"6\" | " + weapon_category_name + "\n")
        ...
        file_out.write("|-\n")
        file_out.write("| [[File:S3 Weapon Main " + weapon + " 2D Current.png|24px|link=" + weapon + "]] [[" + weapon + "]] || "
                       + name + " <small>#" + nameid + "</small> || " + str(power) + " || " + str(rank) +
                       "|| " + title + " || {{UserSplashtag|" + banner)
